...
public class RainWaterTrapped {
    ...
    /**
     * Without any extra space -> using two pointer approach
     *
     * @param arr
     * @return
     */
    private static int trap2(int[] arr) {
        ...
        while (l <= r) {
            if (leftMaxSoFar <= rightMaxSoFar) {
                leftMaxSoFar = Math.max(leftMaxSoFar, arr[l]);
                ans += leftMaxSoFar - arr[l];
                l++;
            } else {
                rightMaxSoFar = Math.max(rightMaxSoFar, arr[r]);
                ans += rightMaxSoFar - arr[r];
                r--;
            }
        }
