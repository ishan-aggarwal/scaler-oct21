...
public class ColorfulNumber {
	public static int colorful(int A) {
		...
		for (int i = 0; i < length; i++) {
			...
			for (int j = i; j < length; j++) {
				product = product * 1l * (input.charAt(j) - 48);
				if (productMap.containsKey(product)) {
					return 0;
				} else {
					productMap.put(product, true);
				}
			}
		}
	...
	public static void main(String[] args) {
		System.out.println("23 is a colorful number : " + colorful(23));
		System.out.println("236 is not a colorful number : " + colorful(236));
	}
}
