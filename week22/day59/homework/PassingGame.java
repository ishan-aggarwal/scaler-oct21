...
public class PassingGame {
	...
	// TC - O(N) - since we are iterating over array C exactly once
	// SC - O(N) - creating a temp stack to hold the current state of id's and
	// returning peek element after A passes
	public int solve(int A, int B, int[] C) {

		Stack<Integer> stack = new Stack<>();

		for (int op : C) {
			if (stack.isEmpty() && op == 0) {
				stack.push(B);
			} else if (!stack.isEmpty() && op == 0) {
				stack.pop();
			} else if (op != 0) {
				stack.push(op);
			}
		}
		return stack.isEmpty() ? B : stack.peek();
	}
